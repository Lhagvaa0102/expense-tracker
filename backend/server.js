...
app.post("/signup", async (req, res) => {
  ...
  try {
    ...
    const newUser =
      await sql`INSERT INTO users(email, password, name) VALUES (${email}, ${password}, ${name}) RETURNING id, email, name`;
    res
      .status(201)
...
app.listen(PORT, () => {
  console.log(`SERVER AJILLAJ BAINA http://localhost:${PORT}`);
});
